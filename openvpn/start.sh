...
# Persist settings for use by Plex
export CONFIG="${CHOSEN_OPENVPN_CONFIG}"
python3 /etc/openvpn/persistEnvironment.py /plex/environment-variables.sh
PLEX_CONTROL_OPTS="--script-security 2 --route-up /etc/openvpn/tunnelUp.sh --route-pre-down /etc/openvpn/tunnelDown.sh"
...
set +u
  ...
  ufwAllowPort ${PLEX_PORT}

  if [[ "${WEBPROXY_ENABLED,,}" == "true" ]]; then
    ufwAllowPort ${WEBPROXY_PORT}
  fi
  if [[ "${UFW_ALLOW_GW_NET,,}" == "true" ]]; then
    ufwAllowPortLong ${PLEX_PORT} ${GW_CIDR}
  else
    ufwAllowPortLong ${PLEX_PORT} ${GW}
  fi
...
if [[ -n "${LOCAL_NETWORK-}" ]]; then
  if [[ -n "${GW-}" ]] && [[ -n "${INT-}" ]]; then
    for localNet in ${LOCAL_NETWORK//,/ }; do
      ...
      if [[ "${ENABLE_UFW,,}" == "true" ]]; then
        ufwAllowPortLong ${PLEX_PORT} ${localNet}
        if [[ -n "${UFW_EXTRA_PORTS-}" ]]; then
          for port in ${UFW_EXTRA_PORTS//,/ }; do
...
exec openvpn ${PLEX_CONTROL_OPTS} ${OPENVPN_OPTS} --config "${CHOSEN_OPENVPN_CONFIG}"
